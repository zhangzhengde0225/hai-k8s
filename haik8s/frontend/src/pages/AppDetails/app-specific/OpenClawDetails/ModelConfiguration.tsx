import { useState } from 'react';
import { Cpu, Plus, Edit2, Trash2, Eye, EyeOff, X } from 'lucide-react';
import toast from 'react-hot-toast';
import client from '../../../../api/client';

interface Props {
  config: any;
  instanceId: number;
  appId: string;
  onConfigUpdate: () => void;
}

export default function ModelConfiguration({ config, instanceId, appId, onConfigUpdate }: Props) {
  const [isEditing, setIsEditing] = useState(false);
  const [showApiKey, setShowApiKey] = useState<Record<string, boolean>>({});
  const [saving, setSaving] = useState(false);

  const providers = config?.models?.providers || {};
  const primaryModel = config?.agents?.defaults?.model?.primary || '';

  const handleDeleteProvider = async (providerName: string) => {
    if (!confirm(`确认删除提供商 "${providerName}" 吗？`)) return;

    setSaving(true);
    try {
      const newProviders = { ...providers };
      delete newProviders[providerName];
      await client.put(`/applications/${appId}/openclaw-config`, {
        instance_id: instanceId,
        models: { providers: newProviders },
      });
      toast.success('已删除');
      onConfigUpdate();
    } catch (err: any) {
      toast.error(err.response?.data?.detail || '删除失败');
    } finally {
      setSaving(false);
    }
  };

  return (
    <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <Cpu className="w-5 h-5 text-gray-600 dark:text-gray-400" />
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">模型配置</h3>
        </div>
        <button
          onClick={() => setIsEditing(!isEditing)}
          className="text-xs text-blue-600 hover:text-blue-700 dark:text-blue-400 flex items-center gap-1"
        >
          {isEditing ? <X size={14} /> : <Edit2 size={14} />}
          {isEditing ? '取消' : '编辑'}
        </button>
      </div>

      {Object.keys(providers).length === 0 ? (
        <div className="text-center py-6 text-gray-500 dark:text-gray-400">
          <p className="mb-2 text-sm">暂未配置模型提供商</p>
          {isEditing && (
            <p className="text-xs">编辑功能即将推出，请在SSH终端中手动编辑配置文件</p>
          )}
        </div>
      ) : (
        <div className="space-y-3">
          {Object.entries(providers).map(([name, provider]: [string, any]) => (
            <div
              key={name}
              className="border border-gray-200 dark:border-slate-700 rounded p-3 bg-gray-50 dark:bg-slate-950/40"
            >
              <div className="flex items-center justify-between mb-2">
                <h4 className="font-medium text-gray-900 dark:text-gray-100 text-sm">{name}</h4>
                {isEditing && (
                  <button
                    onClick={() => handleDeleteProvider(name)}
                    className="text-red-600 hover:text-red-700 dark:text-red-400"
                    disabled={saving}
                  >
                    <Trash2 size={14} />
                  </button>
                )}
              </div>

              <div className="space-y-1 text-xs text-gray-600 dark:text-gray-400">
                <div className="flex justify-between items-start">
                  <span className="flex-shrink-0 w-20">Base URL:</span>
                  <code className="font-mono text-right break-all">{provider.baseUrl}</code>
                </div>
                <div className="flex justify-between items-center">
                  <span className="flex-shrink-0 w-20">API Key:</span>
                  <div className="flex items-center gap-1">
                    <code className="font-mono">
                      {showApiKey[name] ? provider.apiKey : '••••••••••••'}
                    </code>
                    <button
                      onClick={() => setShowApiKey((prev) => ({ ...prev, [name]: !prev[name] }))}
                      className="hover:text-gray-700 dark:hover:text-gray-300"
                    >
                      {showApiKey[name] ? <EyeOff size={12} /> : <Eye size={12} />}
                    </button>
                  </div>
                </div>
                <div className="flex justify-between">
                  <span>模型数量:</span>
                  <span>{provider.models?.length || 0}</span>
                </div>
              </div>

              {/* Model List */}
              {provider.models && provider.models.length > 0 && (
                <div className="mt-2 pt-2 border-t border-gray-200 dark:border-slate-700">
                  <p className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">模型列表:</p>
                  <ul className="space-y-0.5">
                    {provider.models.map((model: any) => (
                      <li
                        key={model.id}
                        className="text-xs text-gray-600 dark:text-gray-400 flex items-center gap-2"
                      >
                        <code className="font-mono">{model.id}</code>
                        <span className="text-gray-400">-</span>
                        <span>{model.name}</span>
                        {`${name}/${model.id}` === primaryModel && (
                          <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-1 rounded text-[10px]">
                            默认
                          </span>
                        )}
                      </li>
                    ))}
                  </ul>
                </div>
              )}
            </div>
          ))}

          {isEditing && (
            <div className="text-center py-4 text-xs text-gray-500 dark:text-gray-400 border-2 border-dashed border-gray-300 dark:border-slate-600 rounded">
              <Plus size={16} className="mx-auto mb-1 text-gray-400" />
              <p>添加和编辑功能即将推出</p>
              <p className="mt-1">请在SSH终端中手动编辑 ~/.openclaw/openclaw.json</p>
            </div>
          )}
        </div>
      )}
    </div>
  );
}
